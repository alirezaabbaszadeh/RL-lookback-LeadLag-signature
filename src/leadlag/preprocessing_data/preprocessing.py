...
def resample_crypto_data(
    ...
    if method is None:
        method_mapping = {"open": "first", "high": "max", "low": "min", "close": "last"}
...
def selected_uni(close_price, df_universe, maximum_coin=50, window_size=210):
    ...
    merged = df_close_month.merge(
        df_universe[["month", "symbols_list"]], left_on="prev_month", right_on="month", how="left"
...
def preprocess_ffill(
    df_close: pd.DataFrame, df_universe: Union[pd.Series, pd.DataFrame], end_date: str = None
