...
def _select_runner(config: Dict[str, Any]):
    ...
    if runner_type == "rl":
        try:
            from leadlag.training.run_rl import run_rl  # type: ignore
...
def _bootstrap_ci(
    values: np.ndarray, alpha: float = 0.05, resamples: int = 2000
) -> tuple[float, float]:
    if len(values) < 2:
...
def run_multiseed(
    ...
    for seed in seeds:
        overrides = {"run": {"seed": seed, "run_name": f"{scenario_name}_seed{seed}"}}
