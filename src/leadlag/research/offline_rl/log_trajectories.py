...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional path to a saved SB3 policy (`model.zip`). Falls back to random "
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    manifest = build_manifest(
        prices, source_path=price_path, extras={"quality": run_quality_checks(prices)}
    )

    log_dir = args.output.resolve().parent
    log_dir.mkdir(parents=True, exist_ok=True)
    log_path = args.log_path or log_dir / "log_trajectories.log"
    config_file = resolve_path("leadlag.configs", "logging_config.yaml")
    setup_logging(
        log_path,
        level=str(args.log_level).upper(),
        config_path=config_file,
        context={"module": "log_trajectories"},
    )
    logger = get_logger(
