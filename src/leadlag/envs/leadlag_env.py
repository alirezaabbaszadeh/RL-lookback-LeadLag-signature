...
class LeadLagEnv(gym.Env):
    ...
    def reset(
        ...
        *,
        ...
        matrix, metrics, extras = self._compute_matrix_and_metrics(
            self._current_index,
            self._current_lookback,
        )
    ...
    def _compute_matrix_and_metrics(
        self,
        index: int,
        lookback: int,
        ...
        if self._prev_matrix is not None and not self._prev_matrix.empty:
            ...
            if len(common_cols) >= 2:
                ...
                if cur_off.size and prev_off.size:
                    try:
                        stability_matrix = float(
                            np.corrcoef(
                                np.nan_to_num(cur_off),
                                np.nan_to_num(prev_off),
                            )[0, 1]
                        )
                    except Exception:
                        stability_matrix = 0.0
                ...
                try:
                    stability_rows = float(
                        np.corrcoef(
                            np.nan_to_num(cur_rows),
                            np.nan_to_num(prev_rows),
                        )[0, 1]
                    )
                except Exception:
                    stability_rows = 0.0
    ...
    def _compute_regime_stats(self, log_returns: pd.DataFrame) -> Dict[str, float]:
        ...
        cross_sectional_vol = (
            float(log_returns.std(axis=1, ddof=0).mean()) if not log_returns.empty else 0.0
        )
    ...
    def get_history_matrices(self) -> pd.Series:
        ...
        data = {
            entry["date"]: entry["matrix"]
            for entry in self.history
            if isinstance(entry["matrix"], pd.DataFrame)
        }
